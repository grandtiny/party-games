...
私人聚会游戏站。首页提供血染钟楼、德州扑克、海龟汤、QQ 农牧场、扫雷、数独和五子棋入口；当前已实现暗流涌动完整本地循环，复用同一服务端规则内核、按行动顺序逐步广播的德扑多人房间和三档确定性单人 AI 对局，服务端持有汤底的多人海龟汤，以及扫雷、数独和支持标准禁手、离线 AI、残局、教学、存档与复盘的五子棋。QQ 农牧场按 7.0 Beta1 Build 20120209.1000 的配置、流程和素材重构，并直接复用平台账号保存长期进度。
...
所有平台账号自动作为庄园好友。好友可访问农场和牧场、浇水、除草、除虫、偷取成熟作物和动物副产品；双账号存档由 SQLite 事务原子更新。同一账号对同季作物或同轮副产品只能偷取一次，作物按原版随机取得 1–5 个并至少保留 60% 产量，动物每次取得 1 个并保留约一半产量。页面还提供农场/牧场排行、留言板、动态和任务。

默认按现实时间成长。开发联调时可设置正数倍率缩短等待时间：
...
packages/manor-v7/               QQ 农牧场 V7 经济、成长、随机事件与存档领域层
...
apps/web/src/games/manor/        QQ 农牧场 V7 场景、商店仓库、好友访问与排行
```
