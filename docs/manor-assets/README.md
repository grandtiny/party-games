...
- 怀旧功能素材导出：`pwsh -File scripts/export-manor-runtime-legacy-features.ps1`
...
| 范围 | 数量 | 当前状态 |
| --- | ---: | --- |
| 原版作物配置/SWF | 86 | 602 个七阶段角色已导出、人工复核并接入；结构解析错误 0 |
| 原版动物配置/SWF | 35 | 210 个六状态角色和 78 个内部辅助类已分离登记并人工复核；210 张运行时 PNG 已接入；结构解析错误 0 |
...
| `crop-runtime-assets.csv` | 86 种作物的 602 张七阶段运行时 PNG、源角色、阶段、哈希和视觉复核状态 |
| `legacy-feature-runtime-assets.csv` | 看门狗、天气、蚊子、便便和 16 张分级窝棚素材的源文件、角色、哈希与运行时路径 |
...
- 完整 86 种作物已按统一七阶段接入 602 张运行时 PNG；水稻纯植株子角色以及水稻、小麦成熟前角色的特殊映射继续保留，逐项来源见 `crop-runtime-assets.csv`。
...
素材准备阶段已完成“批量导出 -> 联系表 -> 人工验收 -> 问题清单”。农场已消费作物台账并接入 86 种作物七阶段、多季、枯萎、照料收益/减产、三档化肥、土地开垦、新手礼包、升级奖励、看门狗、加工厂、固定周四雨天、动态记录和好友恶作剧。162 件通过视觉验收的装扮已接入金币购买、原版有效期、同类启停和永久升级权益。牧场已接入 35 种动物的 210 张六状态运行时 PNG、24 张原版场景和核心控件 PNG、60 条原版可用音频，以及胡萝卜特殊喂养、蚊子便便、动物展示队列和 1–8 级窝棚原图。跨账号收益继续由服务端原子保存；网页动画只做静态原帧的轻量场景表现，不宣称还原 Flash 嵌套时间轴。
