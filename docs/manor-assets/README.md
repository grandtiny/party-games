...
素材准备阶段已完成“批量导出 -> 联系表 -> 人工验收 -> 问题清单”。农场已消费作物台账并接入 86 种作物、原版生长阶段、多季、枯萎、照料收益/减产、三档化肥、土地开垦、新手礼包和升级奖励规则；三档化肥分别来自 `157:Fertilizer`、`164:FertilizerFast` 和 `170:FertilizerVeryFast`，开垦木牌来自 `261:Reclaim`。牧场已接入 35 种动物的 210 张六状态运行时 PNG，以及 24 张原版场景和核心控件 PNG；声音仍按 `animal-audio-policy.csv` 的真实可用变体处理。升级奖励中的装扮目前只记录权益，摆放界面尚未开放；牧场业务循环和好友仍不在当前功能边界。后续接入必须继续消费对应表并按 `integration_policy` 过滤，不直接把原 PHP/Flash 放进运行时，平台账号继续作为唯一账号体系。
